...
| --- | --- | --- | --- |
| `@aria/server` | `packages/runtime/src/{engine,index,runtime}.ts`, `packages/gateway/src/server.ts`, and CLI/runtime bootstrap wiring in `packages/cli/src/{engine,index}.ts` | Server bootstrap helpers, runtime + gateway composition root, daemon lifecycle coordination, and a package-owned Aria Server entry surface | `@aria/runtime`, `@aria/gateway`, and the current `aria` CLI boot path |
...
- `packages/runtime/src/engine.ts` is the current daemon bootstrap shell and should become a compatibility wrapper rather than the long-term composition owner.
