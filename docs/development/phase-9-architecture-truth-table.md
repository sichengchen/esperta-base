...
| `@aria/projects-engine` | `legacy-only compatibility surface` | Compatibility re-exports for project/workspace/job persistence and coordination surfaces | `@aria/projects`, `@aria/workspaces`, `@aria/jobs` | Keep this stable for older imports, but put new project-control behavior under the target packages first. |
