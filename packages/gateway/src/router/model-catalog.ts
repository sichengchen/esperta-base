...
export type ProviderModelCatalogType =
  | "anthropic"
  | "openai"
  | "google"
  | "openrouter"
...
export function getPresetModelList(
  providerType: ProviderModelCatalogType,
  providerId?: string,
): string[] | null {
...
export async function fetchModelList(
  ...
  if (providerType === "google") {
    ...
    return json.models.map((model) => model.name.replace(/^models\//, "")).sort();
  }
...
/**
 * Look up PI-mono model metadata for a provider model ID.
 * Returns null when the model is not in PI-mono's registry or is provider-specific.
 */
export function lookupModelMeta(
  ...
  try {
    const models = (getModels as (provider: string) => { id: string; maxTokens: number }[])(
      providerType,
    );
