...
[ClawHub](https://clawhub.ai) is SA's skill marketplace. The integration is a bundled skill (`src/engine/skills/bundled/clawhub/`) that delegates to the `clawhub` CLI (`npm i -g clawhub` or `pnpm add -g clawhub`).

The skill instructs the agent to run CLI commands via `exec` with `--workdir ~/.sa` so skills are installed into SA's skills directory:
...
1. User asks to find a skill: agent reads the `clawhub` skill, then runs `clawhub search`.
2. User picks a result: agent runs `clawhub install` with the slug.
3. The skill is installed to `~/.sa/skills/` and discovered on the next registry reload.
...
1. Reads every `.md` and `.ts` file (excluding `.test.ts`) from `src/engine/skills/bundled/*/`.
2. Sorts them alphabetically by directory name.
3. Generates `src/engine/skills/embedded-skills.generated.ts`, which exports a `Record<string, Record<string, string>>` mapping directory names to `{ relative path → content }` for all embedded files.
...
6. **ClawHub expansion**: If no installed skill matches, the agent can read the `clawhub` skill, run `clawhub` CLI commands via `exec`, and use the newly installed skill after the registry reloads.
