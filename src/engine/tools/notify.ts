    ...
    async execute(args) {
      ...
      if (target === "all" || target === "telegram") {
        if (secrets?.botToken && secrets.pairedChatId) {
          ...
          return {
            content: "Telegram not configured. Set bot token and paired chat ID via `sa onboard` or `set_env_secret`.",
            isError: true,
          };
        }
      }
      ...
      if (results.length === 0 && errors.length === 0) {
        return { content: "No connectors configured for notifications. Set up Telegram or Discord via `sa onboard`." };
      }
